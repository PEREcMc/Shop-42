  ...
  paintProduct(product) {
    return `
            <li data-id='${product.id}'>
              ...
              <small><strong>$${product.price}</strong></small>
              <button class='toCart'data-add='add'>Добавить в корзину</button>
            </li>
          `
  }
  ...
  paintCartItem(item) {
    return `
              <li>
                ...
                ${item.title}
                <strong>$${item.price}</strong>
                <img class='imgItem' src='${item.image}'>
                <button class='toCart'data-type='remove' data-id='${item.id}'>Удалить товар</button>
              </li>
            `
  }
